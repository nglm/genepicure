...
In this step, we assume that the data is preprocessed and formatted as required. For more information on the correct preprocessing and formatting, see `genepicure.data.checks` module. We use the same notation conventions as in [BioFindr](https://lab.michoel.info/BioFindr.jl/stable/testLLR/#Summary).
...
This section describes the outputs generated by BioFindr's `findr` function and is inspired by one of its tutorial ([DAG construction](https://lab.michoel.info/BioFindrTutorials/dag.html)) and therefore follows its notations.

#### Posterior probabilities matrix `dP`
...
1     geneA    geneB     0.876543     0.00123       1            12             true
2     geneC    geneD     0.654321     0.04567       3            15             true
3     geneB    geneD     0.432109     0.12345       12           15             true
3      …        …           …            …          …             …               …
```

Once the DAG `G` has been constructed (see below) `dP` can have additional columns depending on the method used to infer the GRN. It could for example contain also `inDAG_heuristic_sort` or `inDAG_greedy_insertion` columns in addition to `inDAG_greedy_edges`.


#### DAG (Directed Acyclic Graph) `G`

A DAG constructed from the posterior probabilities matrix `dP` and an object of the [Graphs](https://juliagraphs.org/Graphs.jl/stable/) package in Julia. Nodes correspond to genes (e.g. `geneA`, `geneB`, etc.), and directed edges to causal regulations between genes.

#### Mapping from node names in `dP` to node IDs in `G`: `name2idx`

A dictionary mapping node names (strings e.g. `geneA`, `geneB`, etc.) in `dP` to node IDs (integers) in `G`.
...
### Retrieving parents and children of a gene
